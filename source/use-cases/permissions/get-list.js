const { isNotFound } = require("entity-checker");
const customStatuses = require("../../configurations/custom-statuses");
const apiResponseHelper = require("../../dependency-injections/utilities/api-response");
const projectMessages = require("../../configurations/project-messages");

class GetListUseCase {
  constructor(repository) {
    this.repository = repository;
  }

  async getPermissions(req, res) {
    try {
      const permissions = await this.repository.find(req.query);

      if (isNotFound(permissions)) {
        return apiResponseHelper(
          res,
          customStatuses.notFound,
          projectMessages.permissions.error.getList
        );
      }

      return apiResponseHelper(
        res,
        customStatuses.ok,
        projectMessages.permissions.success.getList,
        permissions
      );
    } catch (error) {
      console.error(error);
      return apiResponseHelper(
        res,
        customStatuses.internalServerError,
        projectMessages.permissions.error.listFetchFailure
      );
    }
  }
}

module.exports = GetListUseCase;
