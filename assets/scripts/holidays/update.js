$(document).ready(function () {
  ...
  $("#update-holiday-button").on("click", function (event) {
    event.preventDefault();
    handleFormSubmission();
  });

  function handleFormSubmission() {
    const { name, startDate, endDate } = fields;
    ...
    if (description.val() === "") {
      ...
      if (endDateErrors) {
        ...
        $.ajax({
          url: "/api/v1/auth/holiday/update?_id=" + _id,
          method: "PUT",
  ...
  function clearErrors() {
    ...
    $("#update-holiday-button").prop("disabled", false);
  }

  function showLoadingSpinner() {
    $("#update-holiday-button").html(`
  ...
  function hideLoadingSpinner() {
    $("#update-holiday-button").html("Submit");
  }

  function handleResponse(message, toastFunction) {
    $("#update-holiday-form")[0].reset();
    hideLoadingSpinner();
